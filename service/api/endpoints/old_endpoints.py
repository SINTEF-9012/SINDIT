...
def get_exit_parts():
    if FAC_NEED_AUTH.lower() == "true":
        g = py2neo.Graph(FAC_NEO4J_URI, user=FAC_NEO4J_USER, password=FAC_NEO4J_PASS)
    ...
    for node in exit_nodes:
        outlet = (
            py2neo.matching.RelationshipMatch(
                g, nodes={node, node}, r_type="DELIVERS_TO"
            )
            .where(type="OUTLET")
            .first()
        )
...
def get_factory_name():
    ...
    graph_property_node = (
        py2neo.matching.NodeMatcher(g).match("GRAPH_PROP_NODE").first()
    )
