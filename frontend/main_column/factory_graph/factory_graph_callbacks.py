...
def store_selected_element_info(n_clicks, last_click_time_str, tap_node, tap_edge):
    ...
    if tap_node is not None and \
            datetime.fromtimestamp(tap_node['timeStamp'] / 1000.0, tz=pytz.UTC) > last_click_time:
        # Currently selected a node:
        selected_el = GraphSelectedElement.from_tap_node(tap_node)
        new_selected_el_json = selected_el.to_json()
        new_click_time = datetime.fromtimestamp(tap_node['timeStamp'] / 1000.0, tz=pytz.UTC)
    elif tap_edge is not None and \
            datetime.fromtimestamp(tap_edge['timeStamp'] / 1000.0, tz=pytz.UTC) > last_click_time:
        # Currently selected an edge:
        selected_el = GraphSelectedElement.from_tap_edge(tap_edge)
        new_selected_el_json = selected_el.to_json()
        new_click_time = datetime.fromtimestamp(tap_edge['timeStamp'] / 1000.0, tz=pytz.UTC)
    else:
...
def update_node_position(n_clicks, selected_el_json):
    ...
    api_client.patch("/graph/node_position",
                     iri=selected_el.iri,
...
def toggle_layout_saver_visibility(selected_el_json, elements):
    ...
    if selected_el_json is not None:
        ...
        if selected_el.is_node:
            ...
            stored_el_pos = next((element['position']
                                  for element in elements
                                  if element['data'].get('iri') == selected_el.iri),
                                 # Default:
                                 {'x': 0, 'y': 0})
            if selected_el.position_x != stored_el_pos.get('x') or \
                    selected_el.position_y != stored_el_pos.get('y'):
