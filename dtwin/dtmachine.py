...
class dtMachine(object):
    ...
    def run(self, queues_in, queues_out, use_kafka_and_neo4j = True):
        ...
        for q_in in queues_in:
            self._print_debug('----------in queue {0} level {1}'.format(q_in.name, q_in.amount))
            number_of_parts_on_queue += len(q_in.parts)
        
        for q_out in queues_out:
            self._print_debug('----------out queue {0} level {1}'.format(q_out.name, q_out.amount))           
        
        while True:                    
            new_part = None  
            new_amount = 0      
            buffers_in = list(filter(lambda x: x.type == dtTypes.dtTypes.BUFFER, queues_in))
            containers_in = list(filter(lambda x: x.type == dtTypes.dtTypes.CONTAINER, queues_in))        
            ...
            for q_in in containers_in:
                ...
                q_in.monitor.append([self.env.now, q_in.amount])
                self._print_debug(self.type.name + ' {0} consumed {1} amount from {2} at time {3} {4} amount left'.format(
                    self.name, self.amount_in, q_in.name, self.env.now, q_in.store.level))
                
                if RUN_KAFKA_PRODUCER and use_kafka_and_neo4j:
                        data_json_IN = json.dumps({'machine_id': self.name,
                                                    'buffer_id': q_in.name,
                                                    'queue_type': q_in.type.name,
                                                    'sensor_id': self.name+'_IN', 
                                                    'timestamp_ms': int(datetime.datetime.now().timestamp()*1000), #UTC
            ...
            buffers_out = list(filter(lambda x: x.type == dtTypes.dtTypes.BUFFER, queues_out))
            containers_out = list(filter(lambda x: x.type == dtTypes.dtTypes.CONTAINER, queues_out))
            ...
            if new_part != None or new_amount > 0:
                ...
                if new_part != None and len(buffers_out) > 0:                   
                    splits = new_part.split_part(num_splits = len(buffers_out)*self.num_parts_out, use_neo4j=use_kafka_and_neo4j)                
                
                for q_out in buffers_out:
                    self._print_debug(self.type.name + ' {0} tries to put {1} parts on queue {2} that has {3} parts stored at time {4}'.format(
                    ...
                    self._print_debug(self.type.name + ' {0} produced {1} parts on {2} at time {3}'.format(
                ...
                for q_out in containers_out:    
                    self._print_debug(self.type.name + ' {0} tries to put {1} amount on queue {2} that has {3} amount stored at time {4}'.format(
                    self.name, self.amount_out, q_out.name, q_out.amount, self.env.now))
                    yield q_out.store.put(self.amount_out)  # @todo: need to figure out how to yield in dtqueue
                    q_out.amount = q_out.store.level
                    
                    self._print_debug(self.type.name + ' {0} produced {1} amount on {2} at time {3}'.format(
